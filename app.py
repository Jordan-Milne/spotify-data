...
def index():
    ...
    def spot_model(pl_1,pl_2):
        ...
        rounded_list = [ round(elem, 3) for elem in model.predict_proba(Z_test)[:,0] ]
        song_list = list(zip(list(X_test['artist']),list(X_test['track_name']),rounded_list))
        sorted_list = sorted(song_list, key=itemgetter(2))
        middle_index = round((len(sorted_list) - 1)/2)
        user1_songs = [i[:2] for i in sorted_list[-5:]]
        user2_songs = [i[:2] for i in sorted_list[:5]]
        similar_songs = [i[:2] for i in sorted_list[middle_index-2:middle_index+3]]
        return roc_auc, user1_songs, user2_songs, similar_songs
    if request.method == 'POST':
        rocauc, user1_songs_final, user2_songs_final, similar_songs_final = spot_model(args.get('playinglist1'), args.get('playinglist2'))
    # rocauc = '88.5'

    return render_template('index.html', rocauc=rocauc, user1_songs_final=user1_songs_final,user2_songs_final=user2_songs_final,similar_songs_final=similar_songs_final)
